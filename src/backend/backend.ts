...
async function main() {
  ...
  const db = Object.fromEntries(rawEntries.flatMap((rawEntry) => {
  ...
  app.get('/api/package/:packageId', (req: any, res: any) => {
    ...
    const entry = {
      ...fields,
      mydeb_available: Object.fromEntries((fields['Depends']?.split(', ') ?? []).flatMap((alts: any) => alts.split(' | ').map((alt: any) => {
  ...
  app.get('/api/package', (req: unknown, res: any) => {
    ...
    const installed = entries.filter((entry: any) => entry['Status'].endsWith(' installed'));
    res.send(installed);
  });
